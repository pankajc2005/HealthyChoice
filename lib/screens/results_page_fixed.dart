...
class _ResultsPageState extends State<ResultsPage> {
  ...
  Future<void> _loadProductImages(String barcode) async {
    try {
      final images = await ApiService.getProductImages(barcode);
      if (mounted) {
        setState(() {
  ...
  void initState() {
    ...
    // Load product images
    _loadProductImages(widget.barcode);
  ...
  void _runAnalysis(Map<String, dynamic> productData) async {
                                                                                ...
                                                                                child: Stack(
                                                                                  children: [
                                                                                    FutureBuilder<String?>(
                                                                                      future: alternative['image_url'] != null 
                                                                                        ? Future.value(alternative['image_url']) 
                                                                                        : ApiService.getProductImageUrl(
                                                                                            alternative['product_name'] ?? '', 
                                                                                            category: productData['categories']?.toString().split(',').first
                                                                                          ),
                                                                                      builder: (context, snapshot) {
                                                                                        if (snapshot.connectionState == ConnectionState.waiting) {
                                                                                          return Container(
                                                                                        ...
                                                                                        if (snapshot.hasData && snapshot.data != null) {
                                                                                          return Image.network(
                                                                                            ...
                                                                                            errorBuilder: (context, error, stackTrace) {
                                                                                              return _buildPlaceholderImage(double.infinity, 80);
                                                                                            },
                                                                                          );
                                                                                          );
                                                                                    ...
                                                                                    Positioned(
                                                                                      ...
                                                                                      child: Container(
                                                                                        ...
                                                                                        child: Text(
                                                                                          alternative['nutriscore_grade']?.toUpperCase() ?? 'B',
                                                                                          style: const TextStyle(
                                                                                            color: Colors.white,
                                                                                            fontSize: 10,
                                                                                            fontWeight: FontWeight.bold,
          ...
          child: Column(
            ...
            children: [
              ...
              AnimatedCrossFade(
                ...
                secondChild: _buildAnalysisContent(productData, isSafe, matchesPreferences),
                crossFadeState: _analysisExpanded ? CrossFadeState.showSecond : CrossFadeState.showFirst,
  ...
  Widget _buildProductImage(double width, double height) {
    // Try to get the best image in this priority order
    final imageUrl = _productImages['front'] ?? 
                    _productImages['main'] ?? 
                    _productImages['front_thumb'] ??
                    _productImages['main_thumb'];
    ...
    // If no image available yet but we're still loading, show loading indicator
    ...
    // As fallback, use the placeholder
    return _buildPlaceholderImage(width, height);
  }
}
